...
import { PropertyRepositoryMock } from "./infra/repo/propertyRepositoryMock";
...
export class Environments {
  private static _testRepoInstance?: PropertyRepository;
  ...
  static getPropertyRepo(): PropertyRepository {
    const { stage } = this.getEnvs();
    switch (stage) {
      case Stage.TEST:
        if (!this._testRepoInstance) {
          this._testRepoInstance = new PropertyRepositoryMock();
        }
      ...
      default:
        return new PropertyRepositoryMock();
    }
  }
}
